...
| `admin` | `/admin`: events list and form (video upload to storage, schedule, CTA, end URL, simulated chat CSV and names, reminder rules), team members and display names, per-session registrants. Replaces the seed script. | `moderator` | this | 2 |
| `replay` | `/replay/<token>`: the recording with normal controls, `replay` attendance rows, the link for follow-up emails. | `room` | this | 2 |
