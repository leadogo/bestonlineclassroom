...
| `reminders` | Per-event rules (minutes before, subject, body) sent by Postmark from our domain at 50 and 30 minutes before the session, with a send log and a one-click stop. Cron every 5 minutes. Built 2026-09-20 evening, waits for a Postmark token; see `SPEC-reminders.md`. | `data-model` | this | 2 |
...
| `phase-4` | Operate many webinars: weekday schedules, admin and moderator roles with email invitations and per-webinar assignments, the dashboard, per-webinar analytics across sessions with the leadogo mapping, ghost and IP blocks, reactions and @mentions for everyone. Map and module notes in `SPEC-phase4.md` (approved 2026-09-20 late). | phases 1 to 3 | this | 4 |
