...
export async function GET(request: Request) {
  ...
  for (const row of data ?? []) {
    ...
    const at_pitch = (att.data ?? []).filter((a) => (a.registrant as unknown as { source: string }).source !== "test" && new Date(a.joined_at as string).getTime() <= pitchAt && new Date(a.last_seen_at as string).getTime() >= pitchAt).length;
    const booked = await db().from("bookings").select("id", { count: "exact", head: true }).eq("event_id", event.id).eq("session_date", s.date);
    sessions.push({ date: s.date, weekday: s.start.getUTCDay(), at_pitch, booked: booked.count ?? 0 });
  }
