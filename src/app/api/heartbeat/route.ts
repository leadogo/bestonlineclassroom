...
/** POST { token, offset, params? }: on open, every 30 s, and on pagehide. One attendance row per registrant per session. */
export async function POST(request: Request) {
  let b: { token?: string; offset?: number; params?: Record<string, string> };
  try {
  ...
  const prev = await db().from("attendance").select("seconds_watched, max_offset, last_seen_at, params").eq("registrant_id", id).eq("session_date", session_date).eq("kind", "live").maybeSingle();
  ...
  const { error } = await db().from("attendance").upsert({ registrant_id: id, session_date, kind: "live", ...next, params }, { onConflict: "registrant_id,session_date,kind" });
  if (error) {
