...
const HISTORY = 300;
const SELECT = "id, author_name, role, body, offset_seconds, reactions, created_at";

async function registrant(token: string) {
  if (!TOKEN_RE.test(token)) return null;
  const { data } = await db().from("registrants").select("id, event_id, session_date, first_name, email, blocked_at, source").eq("token", token).maybeSingle();
  return data;
}
...
export async function GET(request: Request) {
  ...
  const scope = () => db().from("chat_messages").select(SELECT).eq("event_id", r.event_id).eq("session_date", r.session_date);
  ...
  return Response.json({ new: news, updated, now: new Date().toISOString() }, { headers: { "cache-control": "no-store" } });
}

/** POST { token, body, offset }: a real attendee message, relayed to Slack in the background. */
export async function POST(request: Request) {
  let b: { token?: string; body?: string; offset?: number };
  try {
  ...
  if (!r) return Response.json({ error: "Not found" }, { status: 404 });
  if (r.blocked_at) return Response.json({ error: "Chat is unavailable." }, { status: 403 });
  ...
  const ins = await db()
    .from("chat_messages")
    .insert({ event_id: r.event_id, session_date: r.session_date, registrant_id: r.id, author_name: r.first_name, role: "attendee", body, offset_seconds: offset })
    .select(SELECT)
    .single();
  if (ins.error) {
