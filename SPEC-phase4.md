...
Status: **draft for William's review, 2026-09-20 late.** Decisions taken in the spec chat: moderators moderate
...
Build order: `schedules` → `roles-team` → `dashboard` → `chat-moderation` and `chat-social` in parallel.
...
2. **leadogo**: Funnel Performance reads `GET /api/metrics` instead of EasyWebinar's API (change in
   leadogo-app); decide whether the room-count cron reads it too.
